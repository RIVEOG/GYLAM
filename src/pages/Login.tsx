    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-100">Gylam Panel</h1>
          <p className="text-sm text-slate-500">Game Server Management</p>
        </div>
        <form onSubmit={handleSubmit} className="card space-y-4 p-6">
          ...
          <button type="submit" className="btn btn-primary w-full" disabled={loading}>{loading ? 'Signing in...' : <>Sign In <ArrowRight size={16} /></>}</button>
          <p className="text-center text-sm text-slate-500">No account? <Link to="/register" className="font-medium text-blue-400 hover:text-blue-300">Create one</Link></p>
        </form>
