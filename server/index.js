...
app.post('/api/auth/register', async (req, res) => {
  ...
  const user = { id: randomUUID(), username, email: email.toLowerCase(), password_hash: hash, is_admin: users.length === 0, created_at: new Date().toISOString() };
...
function readNodes() { return JSON.parse(readFileSync(NODES_FILE, 'utf-8')).nodes || []; }
function writeNodes(nodes) { writeFileSync(NODES_FILE, JSON.stringify({ nodes }, null, 2)); }
...
app.listen(PORT, () => { console.log(`[Gylam Panel] API running on :${PORT}`); console.log('[Gylam Panel] Made with Nethost Team'); });
