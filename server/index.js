...
app.get('/api/health', (_req, res) => {
  res.json({ status: 'ok', panel: 'Gylam Panel', made: 'Nethost Team' });
});
...
// ── Create admin (from install.sh or panel) ─────────────
app.post('/api/admin/create', async (req, res) => {
  ...
  if (existingIdx !== -1) {
    users[existingIdx].is_admin = true;
    writeUsers(users);
    return res.json({ user: safeUser(users[existingIdx]), message: 'Promoted to admin' });
  }
  const hash = await bcrypt.hash(password, 10);
  const user = {
    id: randomUUID(),
...
function readNodes() {
  return JSON.parse(readFileSync(NODES_FILE, 'utf-8')).nodes || [];
}
function writeNodes(nodes) {
  writeFileSync(NODES_FILE, JSON.stringify({ nodes }, null, 2));
}

app.get('/api/nodes', auth, (_req, res) => {
  res.json({ nodes: readNodes() });
});
...
app.post('/api/nodes', auth, adminOnly, (req, res) => {
  ...
  const node = {
    id: randomUUID(),
    name: req.body.name,
    ip_assigned: req.body.ip_assigned,
    status: 'offline',
    location: req.body.location || '',
    ram_total_mb: req.body.ram_total_mb || 4096,
    disk_total_mb: req.body.disk_total_mb || 20480,
    cpu_total_percent: req.body.cpu_total_percent || 200,
    created_at: new Date().toISOString(),
  };
  nodes.push(node);
  writeNodes(nodes);
...
app.post('/api/nodes/:id/heartbeat', (req, res) => {
  ...
  nodes[idx].status = req.body.status || 'online';
  writeNodes(nodes);
  res.json({ success: true, status: nodes[idx].status });
});

app.delete('/api/nodes/:id', auth, adminOnly, (req, res) => {
  writeNodes(readNodes().filter((n) => n.id !== req.params.id));
  res.json({ success: true });
});

// ── Start ───────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`[Gylam Panel] API running on :${PORT}`);
  console.log(`[Gylam Panel] Made with Nethost Team`);
});
